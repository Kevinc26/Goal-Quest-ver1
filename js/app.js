// ==================== GOALQUEST - APP.JS ====================
// Versión: 2.1 - SISTEMA DE LOGROS + MÓDULO DE PODER
// ============================================================
...
const AchievementSystem = {
    achievements: [
        ...
        { 
            ...
            icon: '🌅',
            color: '#4dff91',
            condition: (state) => state.stats.totalTasksCompleted >= 1,
            onUnlock: () => PowerFeedback.showMessage('Tu viaje comienza ahora.', 'var(--primary)')
        },
        { 
            id: 'task_10', 
            name: '⚡ Diez Pasos', 
            ...
            icon: '⚡',
            color: '#6c63ff',
            condition: (state) => state.stats.totalTasksCompleted >= 10,
            onUnlock: () => PowerFeedback.showMessage('La constancia es tu arma.', 'var(--secondary)')
        },
        { 
            id: 'task_50', 
            name: '🔥 Cincuenta Batallas', 
            description: 'Completa 50 tareas.',
            icon: '🔥',
            color: '#ff6b6b',
            condition: (state) => state.stats.totalTasksCompleted >= 50,
            onUnlock: () => PowerFeedback.showMessage('Ya nada te detiene.', 'var(--danger)')
        },
        { 
            id: 'task_100', 
            name: '👑 Leyenda Viva', 
            ...
            icon: '👑',
            color: '#ffd700',
            condition: (state) => state.stats.totalTasksCompleted >= 100,
            onUnlock: () => PowerFeedback.showMessage('Tu nombre será recordado.', 'var(--gold)')
        },
        ...
        { 
            ...
            icon: '🔥',
            color: '#ffd166',
            condition: (state) => state.stats.dailyStreak >= 7,
            onUnlock: () => PowerFeedback.showMessage('Siete días. Siete victorias.', 'var(--warning)')
        },
        { 
            id: 'streak_30', 
            ...
            icon: '🌋',
            color: '#ffd700',
            condition: (state) => state.stats.dailyStreak >= 30,
            onUnlock: () => PowerFeedback.showMessage('Treinta días. Un hábito. Una leyenda.', 'var(--gold)')
        },
        ...
        { 
            id: 'first_region', 
            name: '🗺️ Explorador', 
            description: 'Completa tu primera región.',
            icon: '🗺️',
            color: '#4ecdc4',
            condition: (state) => Object.keys(state.completedRegions || {}).length >= 1,
            onUnlock: () => PowerFeedback.showMessage('Un mundo se abre ante ti.', 'var(--info)')
        },
        { 
            id: 'all_regions', 
            name: '🌍 Conquistador', 
            description: 'Completa todas las regiones.',
            icon: '🌍',
            color: '#ffd700',
            condition: (state) => Object.keys(state.completedRegions || {}).length >= 8,
            onUnlock: () => PowerFeedback.showMessage('Has conquistado tu mundo interior.', 'var(--gold)')
        },
        
        // 🌟 LOGROS DE JEFES
        { 
            id: 'first_boss', 
            name: '⚔️ Cazador de Dragones', 
            description: 'Derrota a tu primer jefe.',
            icon: '⚔️',
            color: '#ff6b6b',
            condition: (state) => (state.defeatedBosses?.length || 0) >= 1,
            onUnlock: () => PowerFeedback.showMessage('El primer monstruo cae. Tu confianza crece.', 'var(--danger)')
        },
        { 
            id: 'all_bosses', 
            name: '👹 Matadragones', 
            description: 'Derrota a todos los jefes.',
            icon: '👹',
            color: '#ffd700',
            condition: (state) => (state.defeatedBosses?.length || 0) >= 8,
            onUnlock: () => PowerFeedback.showMessage('No quedan monstruos. Solo tú.', 'var(--gold)')
        },
        ...
        { 
            ...
            onUnlock: () => PowerFeedback.showMessage('Tu poder comienza a manifestarse.', 'var(--secondary)')
        },
        { 
            id: 'level_10', 
            name: '🌟 Veterano', 
            ...
            onUnlock: () => PowerFeedback.showMessage('Ya no eres el mismo que empezó.', 'var(--warning)')
        },
        { 
            id: 'level_25', 
            name: '✨ Maestro', 
            ...
            onUnlock: () => PowerFeedback.showMessage('La maestría es silenciosa. Tú la alcanzaste.', 'var(--gold)')
        },
        
        // 🌟 LOGROS DE DÍA COMPLETADO
        { 
            id: 'first_complete_day', 
            name: '🏆 Día Perfecto', 
            description: 'Completa todas tus tareas diarias por primera vez.',
            icon: '🏆',
            color: '#ffd700',
            condition: (state) => state.stats.daysCompleted >= 1,
            onUnlock: () => PowerFeedback.showMessage('Hoy venciste. Mañana también.', 'var(--gold)')
        },
        { 
            id: 'complete_7_days', 
            name: '📅 Semana Perfecta', 
            description: 'Completa 7 días de tareas.',
            icon: '📅',
            color: '#ffd700',
            condition: (state) => state.stats.daysCompleted >= 7,
            onUnlock: () => PowerFeedback.showMessage('Una semana impecable. Eres imparable.', 'var(--gold)')
        }
    ],
    ...
    showNotification(achievement) {
        ...
        notification.innerHTML = `
            <div style="display: flex; align-items: center; gap: 15px;">
                ...
                <div style="flex: 1;">
                    <div style="color: ${achievement.color}; font-size: 10px; margin-bottom: 5px; letter-spacing: 2px;">
                        🏆 LOGRO DESBLOQUEADO
                    </div>
                    <div style="color: white; font-size: 14px; font-weight: bold; margin-bottom: 5px;">
                        ${achievement.name}
                    </div>
                    <div style="color: #aaa; font-size: 11px;">
                        ${achievement.description}
                    </div>
                    <div style="color: ${achievement.color}; font-size: 11px; margin-top: 8px;">
                        +50 EXP
                    </div>
                </div>
            </div>
            ...
            <div class="game-screen active">
                ...
                <div style="display: flex; align-items: center; gap: 30px; margin-bottom: 40px; width: 100%; max-width: 1000px;">
                    ...
                    <div>
                        ...
                        <p style="color: #aaa; font-size: 14px;">
                            Has desbloqueado <span style="color: var(--gold); font-weight: bold;">${unlocked}</span> de <span style="color: var(--gold);">${total}</span> logros
                        </p>
                        <div style="width: 300px; height: 8px; background: rgba(255,215,0,0.2); margin-top: 15px; border: 1px solid var(--gold);">
                            <div style="width: ${(unlocked/total)*100}%; height: 100%; background: var(--gold);"></div>
                        </div>
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(350px, 1fr)); gap: 25px; width: 100%; max-width: 1200px;">
                    ${this.achievements.map(ach => {
                        ...
                        const maxProgress = {
                            'task_10': 10, 'task_50': 50, 'task_100': 100,
                            'streak_7': 7, 'streak_30': 30,
                            'level_5': 5, 'level_10': 10, 'level_25': 25,
                            'all_regions': 8, 'all_bosses': 8,
                            'complete_7_days': 7
                        }[ach.id] || 1;
                        ...
                        return `
                            <div style="
                                background: ${isUnlocked ? 'rgba(255,215,0,0.1)' : 'rgba(26,26,46,0.95)'};
                                border: 2px solid ${isUnlocked ? ach.color : '#3a3a5a'};
                                padding: 20px;
                                display: flex;
                                gap: 20px;
                                position: relative;
                                transition: all 0.3s ease;
                                box-shadow: ${isUnlocked ? `0 0 30px ${ach.color}20` : 'none'};
                            ">
                                <div style="
                                    font-size: 48px;
                                ...
                                <div style="flex: 1;">
                                    <div style="
                                        color: ${isUnlocked ? ach.color : '#aaa'};
                                        font-size: 12px;
                                        font-weight: bold;
                                        margin-bottom: 8px;
                                    ">${ach.name}</div>
                                    ...
                                    ${!isUnlocked ? `
                                        <div style="margin-top: 10px;">
                                            <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                                                <span style="color: #aaa; font-size: 9px;">PROGRESO</span>
                                                <span style="color: ${ach.color}; font-size: 9px;">${progress}/${maxProgress}</span>
                                            </div>
                                            <div style="width: 100%; height: 6px; background: rgba(0,0,0,0.5); border: 1px solid #3a3a5a;">
                                                <div style="width: ${progressPercent}%; height: 100%; background: ${ach.color};"></div>
                                            </div>
                                        </div>
                                    ` : `
                                        <div style="color: ${ach.color}; font-size: 10px; margin-top: 10px;">
                                            ✓ DESBLOQUEADO
                                        </div>
                                    `}
                                </div>
                ...
                <div style="text-align: center; margin-top: 40px;">
                    <button class="ff-button" onclick="window.showScreen('world')" style="background: var(--gold); border-bottom-color: #b8860b;">
                        <i class="fas fa-map"></i> CONTINUAR AVENTURA
                    </button>
                </div>
            </div>
        `;
