...
export default function DailyScreen() {
  ...
  return (
    <div className="game-screen active">
      <button type="button" className="ff-button" onClick={() => setScreen("world")} style={{ marginBottom: "20px" }}>← BACK</button>
      ...
      <section style={{ width: "100%", maxWidth: "800px", margin: "0 auto 30px", padding: "20px", border: "2px solid var(--primary)", borderRadius: "15px", background: "rgba(10, 10, 24, 0.78)", boxShadow: "0 0 20px rgba(77, 255, 145, 0.08)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "15px", flexWrap: "wrap" }}>
          ...
          <div style={{ flex: "1 1 320px", textAlign: "left" }}>
            <h3 style={{ color: "var(--primary)", fontSize: "14px", marginBottom: "8px", textAlign: "left" }}>FORGE YOUR OWN QUEST</h3>
            <p style={{ color: "var(--text-muted)", fontSize: "10px", lineHeight: 1.75, textAlign: "left" }}>Create a quest around what actually matters to you today. It counts exactly like a suggested daily quest.</p>
          </div>
          {!forgeOpen ? (
        ...
        {forgeOpen ? (
          <form onSubmit={forgeCustomQuest} style={{ marginTop: "22px", borderTop: "1px solid rgba(255,255,255,0.18)", paddingTop: "20px" }}>
            <label style={{ display: "block", color: "var(--warning)", fontSize: "10px", marginBottom: "10px", textAlign: "left" }}>WHAT WILL YOU DO?</label>
            <input type="text" value={customQuestText} onChange={(event) => setCustomQuestText(event.target.value)} placeholder="e.g. Read 20 pages" maxLength={100} autoFocus style={fieldStyle} />
            ...
            <div style={{ marginTop: "20px" }}>
              ...
              <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                ...
                <button type="button" className="ff-button" onClick={() => setCustomQuestMode("timer")} style={{ flex: "1 1 220px", background: customQuestMode === "timer" ? "var(--warning)" : "#39394f", color: customQuestMode === "timer" ? "#111" : "#f2f2f5", borderBottomColor: customQuestMode === "timer" ? "#b8862c" : "#242436" }}>⏰ TIMER</button>
              </div>
            </div>

            {customQuestMode === "timer" ? (
              <div style={{ marginTop: "20px" }}>
                <label style={{ display: "block", color: "var(--warning)", fontSize: "10px", marginBottom: "10px", textAlign: "left" }}>MINUTES</label>
            ...
            <p style={{ color: "var(--text-subtle)", fontSize: "10px", lineHeight: 1.75, margin: "18px 0 5px" }}>Custom quests count toward today's quest goal and award the same 25 EXP as a suggested daily quest.</p>
            <div style={{ display: "flex", justifyContent: "center", gap: "10px", flexWrap: "wrap", marginTop: "15px" }}>
              <button type="submit" className="ff-button" disabled={!canForgeQuest} style={!canForgeQuest ? { opacity: 0.68, cursor: "not-allowed", boxShadow: "none" } : undefined}>⚒️ START QUEST</button>
              <button type="button" className="ff-button" onClick={resetForge} style={{ background: "#555568", color: "white", borderBottomColor: "#343444" }}>CANCEL</button>
            </div>
          </form>
        ) : null}
      </section>
